/**
 * Created by zhongkui on 2016/5/5.
 */
...
faultylabs.MD5 = function (h) {
    ...
    function q(a) {
        for (var b = [], h = 0; h < a.length; h++)if (a.charCodeAt(h) <= 127)b.push(a.charCodeAt(h)); else for (var j = encodeURIComponent(a.charAt(h)).substr(1).split("%"), l = 0; l < j.length; l++)b.push(parseInt(j[l], 16));
        return b
    }
...
module.exports = function(h){
    for (var d = "", e = [], c = faultylabs.MD5(h), f = [], a = 0; a < 16 && a < c.length / 2; a++)  f[a] = parseInt(c.slice(2 * a, 2 * a + 2), 16);
    for (a = 0; a <= 7; a++)e[a] = (f[2 * a] + f[2 * a + 1]) % 62, e[a] += e[a] >= 0 && e[a] <= 9 ? 48 : e[a] >= 10 && e[a] <= 35 ? 55 : 61, d += String.fromCharCode(e[a]);
    return d
};